...
const ImageUpload = (props) => {
  ...
  return (
    <label
      ...
      // group is when hover in parent, child will receive class hover
    >
      <input
        type="file"
        name={name}
        // when you want to upload in other places it will have a diffrent name
        className="hidden-input"
        // input hidden, but we can input
        onChange={() => {}}
        // input not onChange have been warning
        {...rest}
      />
      {progress !== 0 && !image && (
        <div className="loading w-16 h-16 border-8 border-orange-500 border-t-transparent animate-spin absolute z-10 rounded-full"></div>
      )}
      {!image && progress === 0 && (
